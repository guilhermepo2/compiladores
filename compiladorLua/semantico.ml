...
let rec infere_exp amb exp =
  match exp with
    S.ExpInt n    -> (T.ExpInt (fst n, A.TipoNumber),       A.TipoNumber)
    ...
    (* (match v with
       ...
       let nome = fst v in
         (try (match (Amb.busca amb nome) with
          ...
          with Not_found ->
                 let msg = "A variavel " ^ nome ^ " nao foi declarada" in
                 failwith (msg_erro v msg)
         )
...
let rec verifica_cmd amb tiporet cmd =
  ...
  | If (teste, entao, senaose, senao) ->
       ...
       (* Verifica a validade de cada comando do bloco 'senãose', se houver *)
       let senaose1 =
          match senaose with
              None -> None
       ...
       let senao1 =
           ...
           | Some bloco -> Some (List.map (verifica_cmd amb tiporet) bloco)
        in
        If (teste1, entao1, senaose1, senao1)
...
and verifica_fun amb ast =
  ...
  match ast with
    ...
    let corpo_tipado = List.map (verifica_cmd ambfn fn_tiporet) fn_corpo in
      Funcao {fn_nome; fn_tiporet; fn_formais; fn_locais; fn_corpo = corpo_tipado}
