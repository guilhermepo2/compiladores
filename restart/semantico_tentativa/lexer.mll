...
| inteiro as num { INT (int_of_string num, pos_atual lexbuf) }
...
| "print"    { WRITE (pos_atual lexbuf)}

| identificador as id { ID (id, pos_atual lexbuf) }
| '"'        {  let buffer = Buffer.create 1 in
                let pos = lexbuf.lex_curr_p in
                let str = leia_string pos buffer lexbuf in
...
and leia_string posicao buffer = parse
      '"'       { Buffer.contents buffer}
    | "\\t"     { Buffer.add_char buffer '\t'; leia_string posicao buffer lexbuf }
    | "\\n"     { Buffer.add_char buffer '\n';  leia_string posicao buffer lexbuf }
    | '\\' '"'  { Buffer.add_char buffer '"'; leia_string posicao buffer lexbuf }
    | '\\' '\\' { Buffer.add_char buffer '\\'; leia_string posicao buffer lexbuf }
    | novalinha { incr_num_linha lexbuf; leia_string posicao buffer lexbuf }
    | _ as c    { Buffer.add_char buffer c; leia_string posicao buffer lexbuf }
