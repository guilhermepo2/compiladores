...
let rec infere_exp amb exp =
  ...
  | S.ExpOpBin (op, esq, dir) ->
    ...
    in
      (T.ExpOpBin ((op,tinf), (esq, tesq), (dir, tdir)), tinf)
...
let rec verifica_cmd amb tiporet cmd =
  let open A in
  match cmd with
  (* nao verificar o retorno pq isso vai fazer o ocaml assumir que os tipos do match
      sao RETSTAT quando sao na verdade stat
    Retorno exp ->
    (match exp with
  ...
  | If (teste, entao, senao) ->
    ...
    let senao1 =
        match senao with
          None -> None
        | Some Else(Bloco(bloco,_)) -> Some (List.map (verifica_cmd amb tiporet) bloco)
     in
